...
sonar_dist = np.zeros(10 ,dtype=np.float64) 

def sonar_callback(data):

	sonar_dist[0] = data.distance
...
def run_hexapod():
	...
	rospy.init_node("hexapod", anonymous=True)

	rospy.Subscriber("sonar_data", Sonar, sonar_callback)

	while not rospy.is_shutdown():

		rospy.loginfo("Distance: %f", sonar_dist[0])

		if(sonar_dist[0] < 20):
			kit1.servo[0].angle = 45
			sleep(0.1)
		else:
